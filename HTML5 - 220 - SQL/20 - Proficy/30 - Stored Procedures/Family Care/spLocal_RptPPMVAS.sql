...
DROP PROCEDURE [dbo].[spLocal_RptPPMVAS]
GO
...
-----------------------------------------------------------------------------------------------------------------------
-- PPM / VAS Report for Family Care configuration
--
...
-----------------------------------------------------------------------------------------------------------------------
-- ========		====	  		====					=====
-- 1.0.0			2018-08-17		Martin Casalis			Initial Release
-- 1.0.1			2019-08-01		Gustavo Conde			Truncate p_vchRptVarIdList to 4000 chars to match legacy report
--														skip N/A convertion to float
--														replace apostrophe on variables names
-- 1.0.2			2019-08-28		Damian Campana			Capability to filter with the time option 'Last Week'   
-- 1.0.3			2020-10-06		Gonzalo Luc			Remove reference to Local_PG_LineStatus and replace it for NPT table.
-- 1.0.4			2021-08-20		Gonzalo Luc			Fix gaps fill with PR In: Line Normal
--=====================================================================================================================
...
-----------------------------------------------------------------------------------------------------------------------
 --SELECT	
	...
	--	@p_vchRptPLIdList			=	'168',
	--	@p_vchRptProdIdList			=	null,
	--	@p_vchRptProductGrpIdList	=	'BTY MACH 9 [All]',
	--	@p_vchRptPUIdList			=	''	,
	--	@p_vchRptTeamList			=	'',
	--	@p_vchRptShiftList			=	'',
	--	@p_vchRptLineStatusList		=	'900^PR In:EO Shippable|899^PR In:Line Normal|902^PR In:Line Project|901^PR In:Qualification|1039^PR Out:Brand Development|904^PR Out:Brand Project|903^PR Out:EO Non-Shippable|905^PR Out:Line Not Staffed|1040^PR Out:Non-Brand Project|906^PR Out:STNU',--'6593^PR Out:Brand Project',--'6589^PR In:Line Normal',--'PR In:Qualification',
	...
	--	@p_intRptWithDataValidationExtended =	1,
	--	@p_vchRptVariableExclusionPrefix	=	'z_obs',
	--	@p_intRptPercentConfidenceAnalysis	=	1,
	--	@p_intRptPercentConfidencePercent	=	1,
...
DECLARE @p_SamplingReqMgt		VARCHAR(5)		--FLD Testing 2015-03-11	
SELECT @p_SamplingReqMgt		=	'NO'    --FLD Testing 2015-03-11	
...
DECLARE	@vchTimeOption							VARCHAR(50),
		...
		@ConTimeSliceEliminationReason8			VARCHAR(500),  --FLD v4.2FLD-7 2015-03-14
		 -- NPT --
...
DECLARE	@tblListPLFilter	TABLE (
		...
		PLId				INT			,
		ActivePPath			INT			DEFAULT 0, -- 0 - No Prod Path Active; Non-zero - Prod Path Active
		CountOfPaths		INT			DEFAULT 0, -- Count of Execution Paths configured on one Line
		IsFamilyCareLine	INT			DEFAULT 0, -- If this is a Family Care business then get the production from the Converter Production PU
		PLDesc				VARCHAR(50)	)
-----------------------------------------------------------------------------------------------------------------------
--	FILTER: Products
-----------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------
--	Test COUNT for time slices that do not have a test COUNT and logic needs to look for test inside the maximum look-up
--	radius
-----------------------------------------------------------------------------------------------------------------------
...
CREATE TABLE	#ValidVarTimeSlices (
				...
				SplitSpecChangeFlag					INT DEFAULT 0	,	-- Used for debugging only: marks records that have been split at spec change boundaries
				PathId								INT					)	
...
--FLD v4.2FLD-7 2015-03-14  Changed all the reasons below and added #8
-----------------------------------------------------------------------------------------------------------------------
SET	@ConTimeSliceEliminationReason1	=	'One or more samples were taken during this time slice for which this test was applicable but there are no results for it'
SET	@ConTimeSliceEliminationReason2	=	'Since the Test Frequency is 0, the calculated sampling interval would be 0 so the sampling radius defaults to the time slice and there are no samples for which this test was applicable during the time slice'		
SET	@ConTimeSliceEliminationReason3	=	'Since the Sampling Interval is NULL, the calculated sampling interval is NULL so the sampling radius defaults to the time slice and there are no samples for which this test was applicable during this period'
SET	@ConTimeSliceEliminationReason4	=	'Since the Sampling Interval is 0, the calculated sampling interval would be 0 so the sampling radius defaults to the time slice and there are no samples for which this test was applicable during the time slice'
SET	@ConTimeSliceEliminationReason5	=	'There are no samples for which this test was applicable during the time slice so we expanded to the sampling radius and found one or more samples were taken during the sampling radius for which this test was applicable but there are no results for it'	
SET	@ConTimeSliceEliminationReason6	=	'There were no samples for which this test was applicable during the time slice nor in the expanded sampling radius'	
SET	@ConTimeSliceEliminationReason7	=	'The report is weighting by production and this time slice had none'	
SET	@ConTimeSliceEliminationReason8	=	'The number of samples associated with this time slice was insufficient [<2] to calculate the product quality measures'	
--=====================================================================================================================
IF @intPRINTFlag = 1	SET	@intSubSecNumber = @intSubSecNumber + 1
...
-----------------------------------------------------------------------------------------------------------------------
	...
	SELECT	@intRptSampleLessThanMINReportingDays	= ISNULL([OpsDataStore].[dbo].[fnRptGetParameterValue] (@ReportName,'Local_PG_intRptSampleLessThanMINReportingDays')	, 30)
...
-----------------------------------------------------------------------------------------------------------
SELECT @vchAppVersion = App_Version FROM AppVersions WHERE app_name = @vchSP_name
-----------------------------------------------------------------------------------------------------------
...
--	Note: this table is used show the user the filter conditions that were in place when an error was trapped
-----------------------------------------------------------------------------------------------------------------------
...
BEGIN
	...
	FROM	#TempTable	tt
	JOIN	dbo.Prod_Lines_Base	pl WITH(NOLOCK)	ON	tt.ValueINT = pl.PL_Id
	WHERE pl.PL_Desc NOT LIKE @vchRptVariableExclusionPrefix + '%'  --2015-02-22 FLD To eliminate obsoleted lines
	ORDER BY 	pl.PL_Desc	-- so production line filter shows up in alphabetical order on the report header									
END

--IF @Debug = 1 SELECT 'Initial insert into @tblListPLFilter based on users selections...'
--IF @Debug = 1 SELECT '@tblListPLFilter',PL_Desc,lplf.* FROM @tblListPLFilter lplf JOIN dbo.Prod_Lines_Base pl WITH(NOLOCK) ON pl.PL_Id = lplf.PLId
...
--	Business Rule:
--	This is an optional parameter therefore it can be NULL
-- 	Web Page used to select Products: "FilterProducts.asp"
...
--IF @Debug = 1 SELECT '@tblListProductGroupsFilter',* FROM @tblListProductGroupsFilter
--IF @Debug = 1 SELECT '@tblListProductFilter',* FROM @tblListProductFilter
...
--IF @Debug = 1 SELECT '#ListDataSourceFilter',* FROM #ListDataSourceFilter
...
--IF @Debug = 1 SELECT '@tblListCrewFilter',* FROM @tblListCrewFilter
...
--IF @Debug = 1 SELECT '@tblListShiftFilter',* FROM @tblListShiftFilter
...
--IF @Debug = 1 SELECT '@tblListPLStatusFilter',* FROM @tblListPLStatusFilter
...
--20140510 FLD Condition f is not programmed.  Is an unlikely condition, however.
-----------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------
--	a.	IF Production Unit Filter is NOT NULL return the Production Units selected by the user
-----------------------------------------------------------------------------------------------------------------------
--IF @Debug = 1 SELECT 'Production Units selected by the user are...'
--IF @Debug = 1 SELECT '@tblListPUFilter',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId
...
BEGIN	
	...
	--	b. 	IF Production Line is selected but Production Unit IS NULL 
	--		THEN return ALL Production Units that belong to the selected Production Lines 
	--		AND, if Data Source variables have been specified, have the same Variable
	--		Groups as the specified variables, OR, if no Data Source variables have been
	--		specified, have the default Variable Groups.
	-- 		FLD 20140510 Edited the above to clarify
	-------------------------------------------------------------------------------------------------------------------
	...
	BEGIN
		...
		BEGIN
	 		...
	 		--FLD 20140510 Begin insertion of new code
	 		WHERE	pu.PU_Id IN (	SELECT DISTINCT pug1.PU_Id 
	 								FROM PU_Groups pug1 WITH (NOLOCK)
									WHERE pug1.PUG_Desc IN (	SELECT DISTINCT pug2.PUG_Desc 
																FROM PU_Groups pug2 WITH (NOLOCK)
																JOIN Variables v WITH (NOLOCK) ON v.PUG_Id = pug2.PUG_Id
																JOIN #ListDataSourceFilter ldsf WITH (NOLOCK) ON ldsf.VarId = v.Var_Id))
	 		--FLD 20140510 End insertion of new code
			--IF @Debug = 1 SELECT 'Production Units if Production Line is selected, Production Unit IS NULL and Data source is specified...'
			--IF @Debug = 1 SELECT '@tblListPUFilter',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId
		END
		ELSE
		BEGIN
			...
			WHERE	tfv.TableId = @intTableId
				...
				AND	tfv.Value = 'Yes'
				AND	pg.PU_Id > 0
			--IF @Debug = 1 SELECT 'Production Units if Production Line is selected, Production Unit IS NULL and Data source is NOT specified...'
			--IF @Debug = 1 SELECT '@tblListPUFilter',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId
		END
	END
	ELSE
	BEGIN
		...
		BEGIN
		 ...
		 --	IF @Debug = 1 SELECT 'Production Units if Production Line is NULL, and Product OR Product Group IS NOT NULL...'
			--IF @Debug = 1 SELECT '@tblListPUFilter',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId
		...
		BEGIN
			...
			WHERE	tfv.TableId = @intTableId
				...
				AND	tfv.Value = 'Yes'
				AND	pg.PU_Id > 0
			--IF @Debug = 1 SELECT 'Production Units if Production Line, Product and Product Group are all NULL and Default Variable Groups is NOT NULL...'
			--IF @Debug = 1 SELECT '@tblListPUFilter',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId
		END
	END
END
...
BEGIN
		 ...
		 --	IF @Debug = 1 SELECT '@tblListPLFilter was empty so we backfilled it from @tblListPUFilter...'
			--IF @Debug = 1 SELECT '@tblListPLFilter',PL_Desc,lplf.* FROM @tblListPLFilter lplf JOIN dbo.Prod_Lines_Base pl WITH(NOLOCK) ON pl.PL_Id = lplf.PLId
END
--=====================================================================================================================
IF @intPRINTFlag = 1	SET	@intSubSecNumber = @intSubSecNumber + 1
...
--  NOTE: We convert this production value to MSU. The conversion to MSU will be done using the conversion factor on the spec 
--  variable.

--	The production variable has to come from either the PU with the FPQ samples, or the PU that is that production point for the 
--	execution path that the PU with the FPQ samples is in. 

--	For each PL in the report the code must do the following to find the production point
--	1. Get Execution Path for the PL
--	2. Find the PU that is the production point for the execution path
--	3. Get the variable associated with the production point from the Prod_Units_Base table.
...
--IF @Debug = 1 SELECT '@tblPUOriginalSamples',* FROM @tblPUOriginalSamples

-----------------------------------------------------------------------------------------------------------------------
--	Get the Production Execution Paths for the Lines selected for the report and determine which ones were ACTIVE
...
--AND pep.Is_Line_Production = 1 --v4.2FLD CHG#4
AND pu.Production_Variable IS NOT NULL --v4.2FLD 2015-02-22 To eliminate unnecessary PU's from table

-- What PL_Id does each Prod Unit involved in the Path belongs to:
...
--IF @Debug = 1 SELECT '@tblProdPlanPath',* FROM @tblProdPlanPath
...
WHERE  		--(pps.End_Time >= @p_vchRptStartDateTime OR pps.End_Time IS NULL) 
             ...
             --FLD 20140510 Test the following simplification
             pps.Start_Time <=	@p_vchRptEndDateTime
			 AND (pps.End_Time > @p_vchRptStartDateTime OR End_Time IS NULL)
			 AND pep.PL_Id IN (SELECT PLId FROM @tblListPLFilter)

--IF @Debug = 1 SELECT '@tblProdPlanActive',* FROM @tblProdPlanActive

-----------------------------------------------------------------------------------------------------------------------
-- Flag the Lines that have at least one Production Execution Path defined.  The ones that have none are set by the 
-- field's default value of 0 already.
-- 20140510 FLD Edtied the above to make it clearer.
-----------------------------------------------------------------------------------------------------------------------
UPDATE @tblListPLFilter
	SET ActivePPath = 1 --pp.PathId  --NOTE: This is just a marker that there is at least 1 path.  There may be others 
								     --besides this ID.  20140510 FLD Changed this to 1 because having a PathId was 
								     --potentially misleading if there were more than one path.
...
--IF @Debug = 1 SELECT '@tblListPLFilter after getting production path info updated...'
--IF @Debug = 1 SELECT '@tblListPLFilter',PL_Desc,lplf.* FROM @tblListPLFilter lplf JOIN dbo.Prod_Lines_Base pl WITH(NOLOCK) ON pl.PL_Id = lplf.PLId

-----------------------------------------------------------------------------------------------------------------------
-- By now we have on the @tblListPUFilter only the Production Units that have the DefaultPUG UDP Configured. 
-- The following logic is built to meet the business rules :
...
--IF @Debug = 1 SELECT '@tblListPUFilter after dump and repopulation from Rule 1a [only one production execution path] with all units on path...'
--IF @Debug = 1 SELECT '@tblListPUFilter--Repop 1a',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId
...
--IF @Debug = 1 SELECT '@tblListPUFilter after dump and repopulation from Rule 1b [no production execution paths] with all units on line...'
--IF @Debug = 1 SELECT '@tblListPUFilter--Repop 1b',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId

-----------------------------------------------------------------------------------------------------------------------
-- Rule 2. For the Lines that have more than 1 Active Production Path, insert whatever is on the Execution Paths. 
-----------------------------------------------------------------------------------------------------------------------
INSERT INTO @tblListPUFilter (
 								PLId,
 								PUId)
...
--IF @Debug = 1 SELECT '@tblListPUFilter after dump and repopulation from Rule 2 [multiple production execution paths] all units on the paths...'
--IF @Debug = 1 SELECT '@tblListPUFilter--Repop 2',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId

-----------------------------------------------------------------------------------------------------------------------
-- Rule 3. The production units that originally held the samples have to be added to the List with two watchouts :
...
--IF @Debug = 1 SELECT '@tblListPUFilter--Rule3: Adding back in the following units from the original list that were not picked up by the repopulation from the paths information...'
--IF @Debug = 1 SELECT 'Adding back into @tblListPUFilter',PLId,PUId FROM @tblPUOriginalSamples WHERE PUId NOT IN (SELECT PUId FROM @tblListPUFilter)
...
--2015-02-22 FLD Added the following 3 lines to get rid of any obsoleted PU's
DELETE lpuf
FROM @tblListPUFilter lpuf
WHERE lpuf.PUDesc LIKE @vchRptVariableExclusionPrefix + '%'   


--FLD 20140510 Begin insert of new code
-----------------------------------------------------------------------------------------------------------------------
--	Update the HoldSamples
-----------------------------------------------------------------------------------------------------------------------
UPDATE	lpuf
SET HoldSamples = 1
FROM	@tblListPUFilter	lpuf
JOIN	@tblPUOriginalSamples puos
		ON	lpuf.PUId = puos.PUId
--FLD 20140510 End insert of new code		
						
-----------------------------------------------------------------------------------------------------------------------
--	Comment this out for FAMILY CARE !   20140510 FLD ???? But it is NOT commented out!  ????
...
WHERE d.Extended_Info    LIKE	'%Category=FamCare%'  --'Category=FamCare;'  --FLD 20140510

--IF @Debug = 1 SELECT '@tblListPLFilter--FINAL',PL_Desc,lplf.* FROM @tblListPLFilter lplf JOIN dbo.Prod_Lines_Base pl WITH(NOLOCK) ON pl.PL_Id = lplf.PLId

--=============================================================================================================================
-------------------------------------------------------------------------------------------------------------------------------
-- WE have to do this for all the units now, given that we could potentially use the ProductionVarId for one Production Unit
...
--IF @Debug = 1 SELECT '@tblListPUFilter--B4 delete of records w NULL prodution VarID',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId

DELETE lpuf
FROM @tblListPUFilter lpuf
WHERE lpuf.ProductionVarId IS NULL --v4.2FLD 2015-02-22 To eliminate unnecessary PU's from table				

--IF @Debug = 1 SELECT '@tblListPUFilter--After delete of records w NULL prodution VarID',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId

--=============================================================================================================================
--SELECT '@tblListPUFilter',*		FROM	@tblListPUFilter puf 
--SELECT '@tblProdPlanPath ',pp.*,pu.PU_Desc	FROM	@tblProdPlanPath pp JOIN Prod_Units_Base pu ON pp.PUId = pu.PU_Id WHERE PathId = 58
...
END
-----------------------------------------------------------------------------------------------------------------------
--	e. 	If IsProductionPoint = 0 on all selected production units, then test COUNT is used for volume weighting
-----------------------------------------------------------------------------------------------------------------------
IF	(	SELECT	SUM(IsProductionPoint) 
		+ COUNT(ProductionVarId)  --2015-02-22 FLD Added to account for the possibility that a PU upstream of the production point holds FPQ variables
...
-----------------------------------------------------------------------------------------------------------------------
--	Look for Line Status on Local_PG_LineStatus for given PU
-----------------------------------------------------------------------------------------------------------------------	
UPDATE	lpu
...
BEGIN
	-------------------------------------------------------------------------------------------------------------------	
	--	GET table Id for Prod_Units_Base
	-------------------------------------------------------------------------------------------------------------------	
	SELECT	@intTableId = TableId
	FROM	dbo.Tables	WITH (NOLOCK)	
...
--IF @Debug = 1 SELECT '@tblListPUFilter--FINAL',PU_Desc,lpuf.* FROM @tblListPUFilter lpuf JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = lpuf.PUId
...
BEGIN
	...
	SELECT	DISTINCT
			...
			CASE	WHEN	v.Data_Type_Id IN (1, 2, 6, 7)
					...
					ELSE	1
					END,
			4, --v.Extended_Test_Freq,   -- FLD Testing 2015-03-02
			--COALESCE(v.Sampling_Interval,0),  -- FLD Testing 2015-02-22
			32767, --133920,  -- FLD Testing 2015-02-22
			v.SA_Id	
...
--IF @Debug = 1 SELECT '#ListDataSource--After Initial Insert',* FROM #ListDataSource
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			---------------------------------------------------------------------------------------------------------------
			--IF @Debug = 1 SELECT '@tblSourcePUList--B4 Delete', * FROM @tblSourcePUList 
			...
			--IF @Debug = 1 SELECT '@tblSourcePUList--After Delete', * FROM @tblSourcePUList
...
--IF @Debug = 1 SELECT '#ListDataSource--After 2nd Insert & Updates',* FROM #ListDataSource
--=====================================================================================================================
IF @intPRINTFlag = 1	SET	@intSubSecNumber = @intSubSecNumber + 1
...
--IF @Debug = 1 SELECT '#ListDataSource--After IsReportable = 0 DEL',* FROM #ListDataSource
-----------------------------------------------------------------------------------------------------------------------	
--	b.	Eliminate all variables Where Calculation_Name IN (''Average_Flex'', ''Range_Flex'')'
-----------------------------------------------------------------------------------------------------------------------	
DELETE	ls
...
--IF @Debug = 1 SELECT '#ListDataSource--After Calculation DEL',* FROM #ListDataSource
-----------------------------------------------------------------------------------------------------------------------
--	c.	Eliminate all variables that starts with a pre_defined prefix
-----------------------------------------------------------------------------------------------------------------------
DELETE	#ListDataSource
...
--IF @Debug = 1 SELECT '#ListDataSource--After Prefix DEL',* FROM #ListDataSource
-----------------------------------------------------------------------------------------------------------------------
...
--IF @Debug = 1 SELECT '#ListDataSource--After DataType DEL',* FROM #ListDataSource
-----------------------------------------------------------------------------------------------------------------------
...
--IF @Debug = 1 SELECT '#ListDataSource--After SPC DEL',* FROM #ListDataSource
-----------------------------------------------------------------------------------------------------------------------
...
--IF @Debug = 1 SELECT '#ListDataSource--After Criticality DEL',* FROM #ListDataSource
--=====================================================================================================================
IF @intPRINTFlag = 1	SET	@intSubSecNumber = @intSubSecNumber + 1
...
--IF @Debug = 1 SELECT '#ListDataSource--FINAL',* FROM #ListDataSource
...
BEGIN
	...
	FROM	@tblValidTimeSlices	vt 
		JOIN	@tblSchedLineStatus rl	ON 	vt.PLId = rl.PLId
				AND vt.PUId = rl.LineStatusPUId  --v4.2FLD CHG#2
...
--IF @Debug = 1 SELECT '@tblValidTimeSlices w LnStatus',* FROM @tblValidTimeSlices
...
--IF @Debug = 1 SELECT '@tblValidTimeSlices w Crew',* FROM @tblValidTimeSlices
...
JOIN @tblProdPlanActive		ppa  ON ts.PLId = ppa.PLId
								 AND ts.PUId = ppa.PUId  --v4.2FLD CHG#1
...
-- sample.
--IF @Debug = 1 SELECT '@tblValidTimeSlices before -1 update to ActivePathId if NULL...'
--IF @Debug = 1 SELECT '@tblValidTimeSlices w Path--B4-1',PU_Desc,vts.* FROM @tblValidTimeSlices vts JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = vts.PUId
...
--IF @Debug = 1 SELECT '@tblValidTimeSlices after -1 update to ActivePathId if NULL...'
--IF @Debug = 1 SELECT '@tblValidTimeSlices w Path--FINAL',PU_Desc,vts.* FROM @tblValidTimeSlices vts JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = vts.PUId
...
--	20140510 FLD added the following note for clarification
--	NOTE: The @tblValidTimeSlices table has all the extraneous PU's in it still, i.e., PU's that have no FPQ variables
--	      them.  With the insert into #ValidVarTimeSlices below, the JOIN with #ListDataSource gets rid of these
--		  extraneous PU's because the JOIN is on the FPQ VarId's included in this report and these PU's have no none.
-----------------------------------------------------------------------------------------------------------------------
...
--IF @Debug = 1 SELECT '#ValidVarTimeSlices after initial insert...'
--IF @Debug = 1 SELECT '#ValidVarTimeSlices--INITIAL',PU_Desc,vts.* FROM #ValidVarTimeSlices vts JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = vts.PUId

-----------------------------------------------------------------------------------------------------------------------
DELETE		#ValidVarTimeSlices
	WHERE	VarId IS NULL
	
--IF @Debug = 1 SELECT '#ValidVarTimeSlices after deleting records with NULL VarId...'
--IF @Debug = 1 SELECT '#ValidVarTimeSlices--NULL VarId Deletion',PU_Desc,vts.* FROM #ValidVarTimeSlices vts JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = vts.PUId
...
--IF @Debug = 1 SELECT '#ValidVarTimeSlices--B4VOL',PU_Desc,vts.* FROM #ValidVarTimeSlices vts JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = vts.PUId
...
BEGIN
	--IF @Debug = 1 SELECT '@vchRptMinorGroupBy',@vchRptMinorGroupBy,'@vchRptMajorGroupBy',@vchRptMajorGroupBy
	-------------------------------------------------------------------------------------------------------------------
	--	b.	If dbo.Prod_Units_Base.Production_Type = 1 then the production source is a variable and the variable will be 
	--		identified in dbo.Prod_Units_Base.Production_Variable
	--		Only include pu's that have IsProductionPoint = 1
	--  NOTE : If the Line has two production points ( that means two variables where to take the production from)
	-------------------------------------------------------------------------------------------------------------------
	...
	--IF @Debug = 1 SELECT '@tblPUIdwithSamples',pu_desc,puws.* FROM @tblPUIdwithSamples puws JOIN dbo.Prod_Units_Base	pu WITH (NOLOCK) ON pu.PU_Id = puws.PUId

	--IF CHARINDEX('PUId', @vchRptMinorGroupBy) > 0  --FLD CHG#2 Temp Fix
	IF CHARINDEX('PUId', @vchRptMinorGroupBy) >= 0 --FLD CHG#2 Temp Fix
	BEGIN
					...
					SELECT		DISTINCT	tvt.PLId					,
											...
											tvt.TimeSliceEnd			,
											tvt.ActivePathId	   
											--tvt.PathId	--Took back out as part of rescinding the simplificatin change below.	
					FROM    @tblValidTimeSlices tvt
							...
							/* The following didn't work because #ValidVarTimeSlices is split up into spec changes which,
							   for a volume standpoint, we don't care about.  The spec changes splits result in multiple
							   representations of a given time period and overstating of volume.
							--@tblValidTimeSlices tvt  --FLD 20140510 Simplified.
							--JOIN	@tblPUIdwithSamples puws ON puws.PUId = tvt.PUId	--FLD 20140510 Simplified.
							--								 AND  tvt.PLId = puws.PLId  -- 20100902	 --FLD 20140510 Simplified.
							--JOIN    @tblListPUFilter puf ON puws.PUId = puf.PUId   --FLD 20140510 Simplified.
							#ValidVarTimeSlices  tvt   --FLD 20140510 Simplified.
							JOIN 	dbo.Prod_Lines_Base	pl WITH (NOLOCK)
														ON tvt.PLId = pl.PL_Id  --puws.PLId = pl.PL_Id   --FLD 20140510 Simplified.
							JOIN	dbo.Prod_Units_Base	pu	WITH (NOLOCK)
														ON pu.PU_Id = tvt.PUId --ON pu.PU_Id = puws.PUId   --FLD 20140510 Simplified.
							*/
					--IF @Debug = 1 SELECT '#MajorMinorVolumeCount1--PUId',* FROM #MajorMinorVolumeCount
	END
	...
	BEGIN
					...
					--IF @Debug = 1 SELECT '#MajorMinorVolumeCount1--PathId',* FROM #MajorMinorVolumeCount
	END
	ELSE --Minor group is anything BUT PUId or PathId
	BEGIN
					...
					--IF @Debug = 1 SELECT '#MajorMinorVolumeCount1--AllElse',* FROM #MajorMinorVolumeCount
	END
	--------------------------------------------------------------------------------------------------------------------------------
	...
	IF CHARINDEX('PUId', @vchRptMinorGroupBy) >= 0 --FLD CHG#2 Temp Fix
	BEGIN
				-- Business Rule: Each Prod Unit that has its own Production VarId drives it's volume by it's own variable
				UPDATE  #MajorMinorVolumeCount						
					SET MajorMinorVolumeCount = (SELECT	SUM(CONVERT(FLOAT,t.Result) * ISNULL(CONVERT(FLOAT,vs.Target),1000.0) / 1000.0)
					    ...
					    --v4.2FLD CHG#1 CountsForMajor	= (CASE lp.IsProductionPoint WHEN 1 THEN 1 ELSE 0 END)
					    CountsForMajor	= 1  --v4.2FLD CHG#1
				FROM #MajorMinorVolumeCount mm
				JOIN @tblListPUFilter		lp		ON		lp.PLId = mm.PLId
															AND lp.PUId = mm.PUId
				WHERE ProductionVarId IS NOT NULL

				--IF @Debug = 1 SELECT '#MajorMinorVolumeCount2a--PUId',* FROM #MajorMinorVolumeCount
				...
				--IF @Debug = 1 SELECT '#ValidVarTimeSlices2a--PUId',* FROM #ValidVarTimeSlices

				-- Business Rule: For the Prod Units that do not have their own Production Var Id, we have to use the 
				-- production points from the execution paths.
			    UPDATE  #MajorMinorVolumeCount						
					SET MajorMinorVolumeCount = (SELECT	SUM(CONVERT(FLOAT,t.Result) * ISNULL(CONVERT(FLOAT,vs.Target),1000.0) / 1000.0)
													--FROM    @tblListPUFilter lp  --v4.2FLD CHG#2b
													FROM	@tblProdPlanPath pp  --v4.2FLD CHG#2b
													JOIN	dbo.Tests t WITH (NOLOCK)	ON	t.Result_On	> mm.TimeSliceStart
																						AND	t.Result_On <= mm.TimeSliceEnd
																						--AND	t.Var_Id = lp.ProductionVarId  --v4.2FLD CHG#2b
																						AND	t.Var_Id = pp.ProductionVarId  --v4.2FLD CHG#2b
											 											AND	Canceled = 0
											 											AND	ISNumeric(Result) = 1 
													LEFT	JOIN	dbo.Var_Specs	vs	WITH (NOLOCK)
																						--ON lp.ProductionVarId = vs.Var_Id  --v4.2FLD CHG#2b
																						ON pp.ProductionVarId = vs.Var_Id  --v4.2FLD CHG#2b
																						AND	mm.ProdId = vs.Prod_ID
																						AND (t.Result_On > vs.Effective_Date AND 
																						(t.Result_On <= vs.Expiration_Date OR vs.Expiration_Date IS NULL))
													WHERE --lp.IsProductionPoint = 1	AND		--v4.2FLD CHG#2b
													--mm.PLId = lp.PLId							--v4.2FLD CHG#2b	
													mm.ActivePathId = pp.PathId AND				--v4.2FLD CHG#2b
													mm.PLId = pp.PLId AND						--v4.2FLD CHG#2b
													pp.IsProductionPoint = 1					--v4.2FLD CHG#2b												
													),
					    --CountsForMajor	= 0  --v4.2FLD CHG#1
					    CountsForMajor = 1  --v4.2FLD CHG#1
				FROM #MajorMinorVolumeCount mm
				JOIN @tblListPUFilter		lp		ON		lp.PLId = mm.PLId
															AND lp.PUId = mm.PUId
				...
				--IF @Debug = 1 SELECT '#MajorMinorVolumeCount2b--PUId',* FROM #MajorMinorVolumeCount
				...
				UPDATE #ValidVarTimeSlices
							SET TimeSliceVolumeCount = (SELECT	SUM(CONVERT(FLOAT,t.Result) * ISNULL(CONVERT(FLOAT,vs.Target),1000.0) / 1000.0)
													...
													LEFT	JOIN	dbo.Var_Specs	vs	WITH (NOLOCK)
																						...
																						(t.Result_On <= vs.Expiration_Date OR vs.Expiration_Date IS NULL))
													WHERE --lp.IsProductionPoint = 1	AND		--v4.2FLD CHG#2b
													--vts.PLId = lp.PLId						--v4.2FLD CHG#2b	
													vts.PathId = pp.PathId AND					--v4.2FLD CHG#2b
													vts.PLId = pp.PLId AND						--v4.2FLD CHG#2b
													pp.IsProductionPoint = 1					--v4.2FLD CHG#2b											
													)
				FROM 	#ValidVarTimeSlices		vts
				JOIN @tblListPUFilter		lp		ON		lp.PLId = vts.PLId
				...
				--IF @Debug = 1 SELECT '#ValidVarTimeSlices2b--PUId',* FROM #ValidVarTimeSlices
				
				--Begin inserted code for part of v4.2FLD CHG#1  This corrects for any redundant volume.
				UPDATE mmvc1
					SET CountsforMajor = 0
					FROM  #MajorMinorVolumeCount mmvc1						
					JOIN #MajorMinorVolumeCount mmvc2 ON mmvc2.ActivePathId = mmvc1.ActivePathId
						AND mmvc2.ProdId = mmvc1.ProdId
					JOIN @tblProdPlanPath pp ON pp.PathId = mmvc1.ActivePathId	--20140610 FLD
						AND pp.PUId = mmvc1.PUId   --20140610 FLD
					WHERE mmvc1.TimeSliceStart >= mmvc2.TimeSliceStart
					AND mmvc1.TimeSliceEnd <= mmvc2.TimeSliceEnd
					AND mmvc1.PLId = mmvc2.PLId  --FLD CHG#2 Fix
					AND mmvc1.PUId <> mmvc2.PUId
					AND pp.IsProductionPoint = 0  --20140610 FLD
				--End inserted code for part of v4.2FLD CHG#1

				--IF @Debug = 1 SELECT '#MajorMinorVolumeCount2c--PUId',* FROM #MajorMinorVolumeCount
	...
	BEGIN
			...
			UPDATE #ValidVarTimeSlices
				...
				--IF @Debug = 1 SELECT '#ValidVarTimeSlices--PathId',* FROM #ValidVarTimeSlices
				
				--Begin inserted code for part of v4.2FLD CHG#1  This corrects for any redundant volume.
				--IMPORTANT!!!  Need to still come back and assess whether to add JOIN to @tblProdPlanPath here as was done above.
				UPDATE mmvc1
					SET CountsforMajor = 0
					FROM  #MajorMinorVolumeCount mmvc1						
					JOIN #MajorMinorVolumeCount mmvc2 ON mmvc2.ActivePathId = mmvc1.ActivePathId
					...
					AND mmvc1.TimeSliceEnd <= mmvc2.TimeSliceEnd
					AND mmvc1.PLId = mmvc2.PLId  --FLD CHG#2 Fix
					--AND mmvc1.PUId <> mmvc2.PUId --FLD CHG#2 Fix
					--End inserted code for part of v4.2FLD CHG#1
				
				--IF @Debug = 1 SELECT '#MajorMinorVolumeCount2--PathId',* FROM #MajorMinorVolumeCount
				
	END
	ELSE  --Minor group is anything BUT PUId or PathId
	BEGIN
				-- If the Time Slice does not have an Active Production Path:
				-- Use the Production Variable for that PU [if there is one]
				UPDATE  #MajorMinorVolumeCount						
					SET MajorMinorVolumeCount = (SELECT	SUM(CONVERT(FLOAT,t.Result) * ISNULL(CONVERT(FLOAT,vs.Target),1000.0) / 1000.0)
													FROM    @tblListPUFilter lp
													JOIN	dbo.Tests t WITH (NOLOCK)	ON	t.Result_On	> mm.TimeSliceStart
													...
													--lp.IsProductionPoint = 1			--v4.2FLD CHG#2
													lp.ProductionType = 1				--v4.2FLD CHG#2											
													AND lp.ProductionVarId IS NOT NULL	--v4.2FLD CHG#2	
													)
				FROM #MajorMinorVolumeCount mm
				WHERE ActivePathId IS NULL
				
				--IF @Debug = 1 SELECT '#MajorMinorVolumeCount2a--AllElse',* FROM #MajorMinorVolumeCount
				...
				UPDATE #ValidVarTimeSlices
							SET TimeSliceVolumeCount = (SELECT	SUM(CONVERT(FLOAT,t.Result) * ISNULL(CONVERT(FLOAT,vs.Target),1000.0) / 1000.0)
													...
													vts.PUId	=	lp.PUId AND			--v4.2FLD CHG#2
													--lp.IsProductionPoint = 1			--v4.2FLD CHG#2
													lp.ProductionType = 1				--v4.2FLD CHG#2
													AND lp.ProductionVarId IS NOT NULL	--v4.2FLD CHG#2	
													)
				...
				--IF @Debug = 1 SELECT '#ValidVarTimeSlices2a--AllElse',* FROM #ValidVarTimeSlices
				...
				UPDATE  #MajorMinorVolumeCount						
					SET MajorMinorVolumeCount = (SELECT	SUM(CONVERT(FLOAT,t.Result) * ISNULL(CONVERT(FLOAT,vs.Target),1000.0) / 1000.0)
													...
													WHERE mm.PLId	=	pp.PLId	AND
--													mm.PathId = pp.PathId AND
													mm.ActivePathId = pp.PathId   --v4.2FLD CHG#2
													--v4.2FLD CHG#2 Commented out the following
													/*
													pp.PathId IN 
				...
				--IF @Debug = 1 SELECT '#MajorMinorVolumeCount2b--AllElse',* FROM #MajorMinorVolumeCount
				...
				UPDATE #ValidVarTimeSlices
							SET TimeSliceVolumeCount = (SELECT	SUM(CONVERT(FLOAT,t.Result) * ISNULL(CONVERT(FLOAT,vs.Target),1000.0) / 1000.0)
													...
													LEFT	JOIN	dbo.Var_Specs	vs	WITH (NOLOCK)
																						...
																						(t.Result_On <= vs.Expiration_Date OR vs.Expiration_Date IS NULL))
													WHERE vts.PLId	=	pp.PLId	AND
													vts.PathId = pp.PathId      --v4.2FLD CHG#2 Commented back in 5/2
				...
				--IF @Debug = 1 SELECT '#ValidVarTimeSlices2b--AllElse',* FROM #ValidVarTimeSlices
				...
				--IF @Debug = 1 SELECT '#MajorMinorVolumeCount2c--AllElse',* FROM #MajorMinorVolumeCount
				...
				--IF @Debug = 1 SELECT '#ValidVarTimeSlices2c--AllElse',* FROM #ValidVarTimeSlices
				...
				--IF @Debug = 1 SELECT '#MajorMinorVolumeCount2d--AllElse',* FROM #MajorMinorVolumeCount
	...
	-- separate tables.
	-- FRio : 2009-09-18
	-- Only include in the #MajorVolumeCount the Volume from PU's that are flagged as CountsForMajor.
...
END --of IF	@intRptVolumeWeightOption = 0 [which is weight by production volume]

--IF @Debug = 1 SELECT '#MajorMinorVolumeCount--FINAL',* FROM #MajorMinorVolumeCount
--IF @Debug = 1 SELECT '#ValidVarTimeSlices--After VOL Update',PU_Desc,vts.* FROM #ValidVarTimeSlices vts JOIN dbo.Prod_Units_Base pu WITH(NOLOCK) ON pu.PU_Id = vts.PUId
--IF @Debug = 1 SELECT '#MajorVolumeCount',* FROM #MajorVolumeCount ORDER BY PLId, TimeSliceStart
...
BEGIN
	...
	BEGIN
		...
		---------------------------------------------------------------------------------------------------------------
		SET	@intRptVolumeWeightOption = 1 -- Volume weight using test COUNT
		---------------------------------------------------------------------------------------------------------------
...
SET	LSL				= vs.L_Reject,
	...
	--TestFreq 		= ISNULL(COALESCE(vs.Test_Freq, ld.ExtendedTestFreq),0) -- FLD Testing 2015-03-11 -- Avoids TestFreq being NULL  
	TestFreq 		= ISNULL(COALESCE(vs.Test_Freq, 4),0) -- FLD Testing 2015-03-11                   -- Avoids TestFreq being NULL  
...
UPDATE		vt
	SET		LSL				= vs.L_Reject,		
			...
			SpecTestFreq 	= vs.Test_Freq,
			--TestFreq 		= COALESCE(vs.Test_Freq, ld.ExtendedTestFreq)   --FLD testing 2015-03-11 
			TestFreq 		= COALESCE(vs.Test_Freq, 4)						--FLD testing 2015-03-11 
FROM		#ValidVarTimeSlices vt 	WITH (INDEX (ValidVarTimeSlices_PLId_ProdId_VarId_TimeSlicePeriod_Idx))
	JOIN	#ListDataSource	ld		ON 	vt.PLId = ld.PLId
										AND	vt.VarId = ld.VarId
	LEFT JOIN	dbo.Var_Specs vs 	WITH (NOLOCK)
...
BEGIN
	...
	FROM	--fnLocal_NormPPM_GetOfflineQualitySpecs_Test (@intSamplePUId, @intVarId, @dtmTimeSliceStart, @dtmTimeSliceEnd) --FLD Testing 2015-03-11
	        fnLocal_NormPPM_GetOfflineQualitySpecs (@intSamplePUId, @intVarId, @dtmTimeSliceStart, @dtmTimeSliceEnd)    --FLD Testing 2015-03-11
...
--	e.	Eliminate Slices where TestCountResultNOTNULL = 0, TestCountResultNULL = 0 AND SamplingInterval = 0
-----------------------------------------------------------------------------------------------------------------------
--	a.	Eliminate Slices where production count is NULL if volume weighting is set to production COUNT
--		Rationale:	The report is weighting by production and this time slice had none.
-----------------------------------------------------------------------------------------------------------------------
IF	@intRptVolumeWeightOption = 0
BEGIN
	UPDATE		vt
...
IF UPPER(@p_SamplingReqMgt) = 'YES'		--FLD Testing 2015-03-11
BEGIN  --FLD Testing 2015-03-11
-----------------------------------------------------------------------------------------------------------------------
--	b.	Eliminate Slices where TestCountResultNOTNULL = 0 and TestCountResultNULL > 0 
--		Rationale:	Tests were required [indicated by TestCountResultNULL being > 0] but none were done [indicated by 
--					TestCountResultNOTNULL = 0], i.e., applicable tests were not done.
-----------------------------------------------------------------------------------------------------------------------
UPDATE	vt
SET		TimeSliceEliminationFlag	= 1,
...
-----------------------------------------------------------------------------------------------------------------------
--	c.	Eliminate Slices where TestFreq = 0 AND TestCountResultNOTNULL IS NULL AND TestCountResultNULL IS NULL
-- FLD added below 20150303
--		Rationale:	Since the Test Frequency is 0, the overall sampling interval is 0 so the sampling radius defaults 
--					to the time slice and there are no tests applicable during this period.
-----------------------------------------------------------------------------------------------------------------------
  UPDATE	vt
  SET		TimeSliceEliminationFlag	=	1,
...
-- FLD added below 20150303
--		Rationale:	Since the Sampling Interval is 0, the overall sampling interval is 0 so the sampling radius 
--					defaults to the time slice and there are no tests applicable during this period.
-----------------------------------------------------------------------------------------------------------------------
UPDATE	vt
SET		TimeSliceEliminationFlag	=	1,
...
WHERE	vt.TestCountResultNOTNULL IS NULL
	AND	vt.TestCountResultNULL IS NULL
	--AND	vt.TestFreq > 0  --FLD  20150303 Not needed since any test freq that are not >0 have already been eliminated.
	AND	vt.SamplingInterval = 0
	AND	vt.TimeSliceEliminationFlag = 0

-----------------------------------------------------------------------------------------------------------------------
--	d.	Eliminate Slices where Sampling Interval IS NULL AND TestCountResultNOTNULL IS NULL AND TestCountResultNULL IS NULL
-- FLD added below 20150303
--		Rationale:	Since the Sampling Interval is NULL, the overall sampling interval is NULL so the sampling radius 
--					defaults to the time slice and there are no tests applicable during this period.
-----------------------------------------------------------------------------------------------------------------------
UPDATE	vt
SET		TimeSliceEliminationFlag	=	1,
...
FROM	#ValidVarTimeSlices	vt
WHERE	vt.TestCountResultNOTNULL IS NULL  --FLD Addded 20150303  We really only care about the sampling interval 
	AND	vt.TestCountResultNULL IS NULL	--FLD Addded 20150303     if we need to compute a sampling radius
	AND vt.SamplingInterval IS NULL
	AND	vt.TimeSliceEliminationFlag = 0
...
IF	EXISTS	(	SELECT	TimeSliceId
				...
				WHERE	TestCountResultNOTNULL 	IS NULL
					...
					--AND	TestFreq > 0  --FLD  20150303 Not needed since any test freq that are not >0 have already been eliminated.
...
BEGIN
	...
	--	d.	ELIMINATE Time Sliced where MSRTestCountResultNOTNULL = 0 AND MSRTestCountResultNULL > 0
	-- FLD added below 20150303
	--		Rationale:	There were no tests applicable during the time slice so we expanded to the sampling radius and 
	--					found applicable tests that were not done.
	-------------------------------------------------------------------------------------------------------------------
	UPDATE	vt
	SET		TimeSliceEliminationFlag = 1,
	...
	--	e.	ELIMINATE Time Sliced where MSRTestCountResultNOTNULL IS NULL AND MSRTestCountResultNULL IS NULL 
	-- FLD added below 20150303
	--		Rationale:	There were no tests applicable during the time slice nor in the expanded sampling radius.
	-------------------------------------------------------------------------------------------------------------------
	UPDATE	vt
	SET	TimeSliceEliminationFlag = 1,
...
END  --to IF	EXISTS	(	SELECT	TimeSliceId etcetera	--FLD Testing 2015-03-11

END --to IF UPPER(@p_SamplingReqMgt) = 'YES'	--FLD Testing 2015-03-11
...
--=====================================================================================================================
--------------------------------------------------------------------------------------------------------------------------------
-- If the parameter used to give an extra Sheet with the Raw data used to calculate the volume is set to 1
...
BEGIN  
	...
	SELECT	
				...
				JOIN    --v4.2FLD CHG#3 
						--(SELECT PLId, ProductionVarId FROM @tblListPUFilter  
						(SELECT PLId, PUId, ProductionVarId FROM @tblListPUFilter  --v4.2FLD CHG#3 Added PUId to SELECT
							WHERE   --IsProductionPoint = 1 FLD 2015-02-04 to bring in PU's with FPQ results that are not the production point 
							--AND     FLD 2015-02-04 to bring in PU's with FPQ results that are not the production point 
							ProductionVarId IS NOT NULL)	lp	ON 	mm.PLId = lp.PLId
																		AND mm.PUId = lp.PUId --v4.2FLD CHG#3
				...
				--IF @Debug = 1 SELECT '#ExcelVolumeCheck',* FROM #ExcelVolumeCheck
...
BEGIN
				...
				--IF @Debug = 1 SELECT '#MajorGroupList--None' FROM #MajorGroupList
END
ELSE
BEGIN
		...
		--IF @Debug = 1 SELECT 'MajorGroupBySQL = ' + @nvchSQLCommand
		--IF @Debug = 1 SELECT '#MajorGroupList--'  + @vchRptMajorGroupBy,* FROM #MajorGroupList
...
BEGIN
	...
	--IF @Debug = 1 SELECT '#ValidVarTimeSlices--None' FROM #ValidVarTimeSlices
	--IF @Debug = 1 SELECT '#MajorMinorVolumeCount--None' FROM #MajorMinorVolumeCount
...
--IF @Debug = 1 SELECT '#ValidVarTimeSlices--'  + @vchRptMajorGroupBy,* FROM #ValidVarTimeSlices
--IF @Debug = 1 SELECT '#MajorMinorVolumeCount--'  + @vchRptMajorGroupBy,* FROM #MajorMinorVolumeCount
...
BEGIN
	...
	BEGIN
		SET	@nvchSQLCommand3 =	'	FROM	#MajorMinorVolumeCount ' 
						+		'	WHERE	CountsForMajor = 1'  --FLD CHG#2 Fix
						+		'	GROUP BY	MajorGroupId'
	END
	--ELSE  --FLD CHG#2 Fix
	--FLD CHG#2 Fix Start of new inserted code
	ELSE IF @vchRptMinorGroupBy = 'PUId'
	BEGIN
	SET	@nvchSQLCommand3 =	'	FROM	#MajorMinorVolumeCount ' 
						+		'	GROUP BY	MajorGroupId, ' + REPLACE(@vchRptMinorGroupBy, '|', ',')
	END
	ELSE
	--FLD CHG#2 Fix End of new inserted code
	BEGIN
		SET	@nvchSQLCommand3 =	'	FROM	#MajorMinorVolumeCount ' 
						+		'	WHERE	CountsForMajor = 1'  --FLD CHG#2 Fix
						+		'	GROUP BY	MajorGroupId, ' + REPLACE(@vchRptMinorGroupBy, '|', ',')
...
-----------------------------------------------------------------------------------------------------------------------
BEGIN
	--FLD CHG#2 Fix Start of new inserted code 20140507
	IF @vchRptMinorGroupBy = 'PUId'
	BEGIN
		SET	@nvchSQLCommand3 =	'	FROM	#MajorMinorVolumeCount ' 
	...
	ELSE
	BEGIN
	--FLD CHG#2 Fix End of new inserted code 20140507

		SET	@nvchSQLCommand3 =	'	FROM	#MajorMinorVolumeCount ' 
							+	'	WHERE	CountsForMajor = 1'  --FLD CHG#2 Fix
							+	'	GROUP BY	MajorGroupId, ' + REPLACE(@vchRptMajorGroupBy, '|', ',') + ', ' + REPLACE(@vchRptMinorGroupBy, '|', ',')
	END  --FLD CHG#2 Fix 20140507
END
...
--IF @Debug = 1 SELECT 'MinorGroupBySQL = ' + @nvchSQLCommand
--IF @Debug = 1 SELECT '#MinorGroupList--' + @vchRptMinorGroupBy,* FROM #MinorGroupList
...
BEGIN
	...
	--AND   PUId IN (SELECT PUId FROM #ValidVarTimeSlices WHERE TimeSliceEliminationFlag = 0)
	AND   PUId IN (SELECT PUId FROM #ValidVarTimeSlices vts WHERE TimeSliceEliminationFlag = 0 AND vts.ProdId = migl.ProdId) --FLD Added the AND 20140510
	
	--IF @Debug = 1 SELECT '#MinorGroupList--AfterUP',* FROM #MinorGroupList
	--IF @Debug = 1 SELECT '#ValidVarTimeSlices--At MinorGroupList Update', * FROM #ValidVarTimeSlices
END
...
BEGIN
	...
	GROUP BY	vt.MajorGroupId, vt. MinorGroupId, ld.PLId, ld.PUGId, ld.TzFlag, 
				...
				ld.IsNumericDataType, ld.IsOfflineQuality
	ORDER BY	vt.MajorGroupId, vt.MinorGroupId, ld.PLId, ld.PUGId,
				vt.ProductGrpId, ld.VarGroupId, vt.Target
END
ELSE
BEGIN
	INSERT INTO	#CalcPPM (
...
--IF @Debug = 1 SELECT '#ValidVarTimeSlices--After All Updates',* FROM #ValidVarTimeSlices
...
BEGIN
	-------------------------------------------------------------------------------------------------------------------
	--	DELETE records where test COUNT < 2  --FLD v4.2FLD-7  2-15-03-14
	-------------------------------------------------------------------------------------------------------------------
	--IF @Debug = 1 SELECT '#CalcPPM-B4 delete of test count < 2',* FROM #CalcPPM
	
	--Reason #8  beg  --FLD v4.2FLD-7  2-15-03-14
	UPDATE		vt
	SET		TimeSliceEliminationFlag	=	1,
			TimeSliceEliminationReason	=	@ConTimeSliceEliminationReason8
	FROM	#ValidVarTimeSlices	vt JOIN #CalcPPM cp WITH (NOLOCK) ON cp.CalcPPMId = vt.CalcPPMId
	WHERE	vt.TimeSliceEliminationFlag = 0
	AND		(cp.TestCount = 1 OR cp.TestCount = 0 OR cp.TestCount IS NULL)
	
	--IF @Debug = 1 SELECT '#ValidVarTimeSlices-after Reason 8 update',* FROM #ValidVarTimeSlices
	--Reason #8  end  --FLD v4.2FLD-7  2-15-03-14
	
	DELETE	#CalcPPM
	WHERE	TestCount = 1 OR TestCount = 0 OR TestCount IS NULL   --FLD v4.2FLD-7  2-15-03-14
	
	--IF @Debug = 1 SELECT '#CalcPPM-after delete of test count < 2',* FROM #CalcPPM
	
	/*   --FLD v4.2FLD-7  2-15-03-14  consolidated above
	-------------------------------------------------------------------------------------------------------------------
	--	DELETE records where test COUNT = 1
	-------------------------------------------------------------------------------------------------------------------
	IF @Debug = 1 SELECT '#CalcPPM-B4 delete of test count = 1',* FROM #CalcPPM
	DELETE	#CalcPPM
	WHERE	TestCount = 1
	*/   --FLD v4.2FLD-7  2-15-03-14
...
--=====================================================================================================================
-- Calculate square deviation for data where sample < flag = 0 or no historical data has been 
-- found
--=====================================================================================================================
IF @p_vchRptLayoutOption 		=	'VASReport' 
BEGIN
...
--	CharId				)
	...
	--2015-02-22 FLD Marked as potential place to do the delete of #CalcPPM records based on Test Count
...
--2015-02-22 FLD Marking this as a point of interest wrt the VMC volume issue
-----------------------------------------------------------------------------------------------------------------------
...
BEGIN
	...
	GROUP BY	vt.MajorGroupId, vt.MinorGroupId, vt.PLId, ld.PUGId, 
				vt.ProductGrpId, ld.VarGroupId, ld.VarCount,
				vt.LEL, vt.LSL, vt.Target, vt.USL, vt.UEL, vt.LTL, vt.UTL		,
				ld.IsNumericDataType

--IF @Debug = 1 SELECT '@tblVarProductionInterim2-After update of volume from #ValidVarTimeSlices',* FROM @tblVarProductionInterim2
...
--IF @Debug = 1 SELECT '#CalcPPM-After update of VolumeCount',* FROM #CalcPPM
...
--IF @Debug = 1 SELECT '#CalcPPM-After update of SUMVolumeCount',* FROM #CalcPPM
...
-----------------------------------------------------------------------------------------------------------------------
UPDATE	cp
SET	MCSymmetricSpecs 	= 	CASE	WHEN	(CONVERT(DECIMAL(20, 10), USL) - CONVERT(DECIMAL(20, 10), Target)) = (CONVERT(DECIMAL(20, 10), Target) - CONVERT(DECIMAL(20, 10), LSL))
									THEN	1
									ELSE 	0
							END		
FROM	#CalcPPM			cp
...
-----------------------------------------------------------------------------------------------------------------------
UPDATE	#CalcPPM
SET	MCSymmetricSpecs 	= 	Case	WHEN	(CONVERT(DECIMAL(20, 10), USL) - CONVERT(DECIMAL(20, 10), UTL)) = (CONVERT(DECIMAL(20, 10), LTL) - CONVERT(DECIMAL(20, 10), LSL))
									THEN	1
									ELSE 	0
							END		
FROM	#CalcPPM		cp
...
BEGIN
		...
		BEGIN
			...
			INSERT INTO	@tblMetCritPPassAvg (
 			...
 			SELECT	
					...
					--SUM(cp.MetCritActual * cp.VolumeCount / cp.MajorGroupVolumeCount) / COUNT(DISTINCT cp.VarDescRpt) --MFG SVCE v5.0 FLD 20150302
			FROM	#CalcPPM		cp
				JOIN	#MinorGroupList	mi 	ON 	cp.MajorGroupId = mi.MajorGroupId
												AND	cp.MinorGroupId = mi.MinorGroupId
				JOIN	dbo.PU_Groups 	pg 	WITH (NOLOCK)
	...
	BEGIN
		...
		BEGIN
			...
			--  20090415
			-----------------------------------------------------------------------------------------------------------			
			IF @i > 90 --50  --FLD CHG#2 20140508
			BEGIN
				SET @intMinorGroupId = @intMAXMinorGroupId + 1
				SET @vchWarningMsg = 'The Grouping Option selected for this report exceeds the Maximum, so information maybe incomplete and inaccurate!'  --FLD 20140508
			END
			-----------------------------------------------------------------------------------------------------------	
			--	REQUIRED CODE
			-----------------------------------------------------------------------------------------------------------			
...
--		SELECT '#NormPPMRptFinalResultSetDetailInterim >>',* FROM #NormPPMRptFinalResultSetDetailInterim
--		SELECT '#NormPPMRptFinalResultSetDetail >>',* FROM #NormPPMRptFinalResultSetDetail	
		---------------------------------------------------------------------------------------------------------------
		--	INCREMENT COUNTER
		---------------------------------------------------------------------------------------------------------------
	...
	IF @i > 90 --50  --FLD CHG#2 20140508
	BEGIN
		SET @i = 90 --50   --FLD CHG#2 20140508
...
BEGIN
	...
	BEGIN
		UPDATE	@tblVASRptVarAttributes
				SET	PercentTarget = (SampleCount - DefectCount) / SampleCount
	END
...
GO
GRANT EXECUTE ON [dbo].[spLocal_RptPPMVAS] TO OPDBmanager
GRANT EXECUTE ON [dbo].[spCmn_ReportCollectionParsing] TO OPDBManager 
GRANT EXECUTE ON [dbo].[spLocal_RptRunTime] TO OPDBManager
GRANT EXECUTE ON [dbo].[spLocal_RptPPMVAS] TO RptUser
GO