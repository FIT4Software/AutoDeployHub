...
---- 1.2.1			2019-08-01		Gustavo Conde			Truncate p_vchRptVarIdList to 4000 chars to match legacy report
----														skip N/A convertion to float
----														replace apostrophe on variables names
---- 1.2.2			2019-08-28		Damian Campana			Capability to filter with the time option 'Last Week'  
----=====================================================================================================================
...
-----------------------------------------------------------------------------------------------------------------------
 --SELECT	
	...
	--	@p_vchRptPLIdList			=	'557',
	--	@p_vchRptProdIdList			=	null,
	--	@p_vchRptProductGrpIdList	=	null,
	--	@p_vchRptPUIdList			=	''	,
	--	@p_vchRptTeamList			=	'',
	--	@p_vchRptShiftList			=	'',
	--	@p_vchRptLineStatusList		=	'',--'PR In:Qualification',
...
BEGIN
	...
	-- description.
	-------------------------------------------------------------------------------------------------------------------
	SELECT	@intDataTypeId = Data_Type_Id
	...
	FROM	#TempTable	tt
		JOIN	dbo.Phrase	p	WITH (NOLOCK)
								ON	tt.ValueINT = p.Phrase_Id
									AND	p.Data_Type_Id = @intDataTypeId							
END
--=====================================================================================================================
IF @intPRINTFlag = 1	SET	@intSubSecNumber = @intSubSecNumber + 1
...
BEGIN
	...
	BEGIN
		INSERT INTO	@tblSchedLineStatus (
					...
					LineStatusPUId, 
					PLId )
		SELECT	lpgls.Status_Schedule_Id, 
				lpgls.Start_DateTime, 
				COALESCE(lpgls.End_DateTime, @p_vchRptEndDateTime),
				lpgls.Line_Status_Id, 
				lpgls.Unit_Id,
				lp.PLId
		FROM	dbo.Local_PG_Line_Status lpgls	WITH (NOLOCK)
			JOIN	@tblListPUFilter lp	ON	lpgls.Unit_Id = lp.LineStatusPUId
		WHERE	lpgls.Start_DateTime <= @p_vchRptEndDateTime
			AND (lpgls.End_DateTime  >  @p_vchRptStartDateTime 
				OR lpgls.End_DateTime IS NULL)
	END
--	ELSE	
--		TODO write code to retrieve line status from new proficy table
--	END
	-------------------------------------------------------------------------------------------------------------------
	--	c-2.	GET the line status for the time slice
	-------------------------------------------------------------------------------------------------------------------
...
--SELECT '@tblProdPlanActive',* FROM @tblProdPlanActive ORDER BY PPSStartTime
--SELECT '@tblProdPlanPath',* FROM @tblProdPlanPath
--SELECT '@tblListProductFilter',* FROM @tblListProductFilter
--SELECT '@tblValidTimeSlices',* FROM @tblValidTimeSlices where TimeSliceStart = '2011-03-01 19:42:14.000'
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			-- Now it is going to be the AVG vs the weigted volume
			--=================================================================================================================================
			INSERT INTO	@tblMetCritPPassAvg (
					MajorGroupId,
					MinorGroupId,
...
-- SELECT '#ValidVarTimeSlices'						,PO,* FROM 	#ValidVarTimeSlices vts -- WHERE VarId = 25950
