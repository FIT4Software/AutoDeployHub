...
---------------------------------------------------------------------------------------------------------------------------
IF EXISTS (
			SELECT * FROM dbo.sysobjects WITH(NOLOCK)
				WHERE Id = OBJECT_ID(N'[dbo].[spLocal_PositiveRelease]') 
				AND OBJECTPROPERTY(id, N'IsProcedure') = 1					
			)
DROP PROCEDURE [dbo].[spLocal_PositiveRelease]
...
SELECT
		@SP_Name	= 'spLocal_PositiveRelease',
		@Inputs		= 6, 
		@Version	= '1.0.4'  
...
-- Positive Release
--
-- 2019-07-18		Gustavo Conde						Arido Software
-----------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------
-- ========		====	  		====					=====
-- 1.0.0		2019-07-18		Conde Gustavo			Wraped initial version, bassed on 2.9
-- 1.0.1		2019-08-13		N/A						New version convention
-- 1.0.2    		2019-09-02		Damian Campana			Add global parameters Trimm Downtimes (min)
-- 1.0.3		2019-08-28		Damian Campana			Capability to filter with the time option 'Last Week' 
-- 1.0.4		2022-07-04		Britos Marcos			Fix column size "Type" in @QFactorType - PRB0094531
----=====================================================================================================================
--------------------------------------------------[Creation Of SP]-------------------------------------------------------
CREATE PROCEDURE [dbo].[spLocal_PositiveRelease] 
-----------------------------------------------------------------------------------------------------------------------
--DECLARE 
	@in_TimeOption		INT				, 	--	PLID Comma separated values
	@in_StartTime       DATETIME		,	--  Start Time of Sample SET  
	@in_ENDTime         DATETIME		,	--	0 = NO / 1 = YES
	@PLIDList			NVARCHAR(100)	,
	@int_RptGroupBy		INT				,   --	0 = None / 1 = Product / 2 = Process Order 
	@int_ShowDetails	INT				,	--  END Time of Sample SET
	@INT_RptPosMin 		INT				,
	@INT_RptNegMin		INT		
--WITH ENCRYPTION 
AS  
...
-- --Test Data  
-- SELECT  
-- @in_StartTime = '2011-06-09 07:30:00.000',  
-- @in_ENDTime  = '2011-06-10 07:30:00.000',
-- @int_RptGroupBy	= 0,		--0 = None / 1 = Product / 2 = Process Order 
-- @int_ShowDetails = 0,		--0 = NO / 1 = YES
-- @PLIDList		= '',
-- @Owner			= '';
---------------------------------------------------------------------------------------------------------------------  
  
DECLARE @ReportName	NVARCHAR(50) = 'Positive Release Report';
...
--------------------------------------------------------------------------------------------------------  
DECLARE @AttrTests TABLE (  
        PU_Id      		INT,  
        PUG_ID      	INT,  
        PUGDesc      	VARCHAR(50),  
        Result      	VARCHAR(50),   
        Result_On     	DATETIME,  
        Result_OnNeg30  DATETIME,  
        Result_OnPos30  DATETIME,  
        Entry_On     	DATETIME,  
        Var_ID      	INTeger 
)  
--------------------------------------------------------------------------------------------------------  
...
--====================================================================================================  
PRINT ' - Retrive iODS global parameters'  
------------------------------------------------------------------------------------------------------  
...
-----------------------------------------------------------------------------------------------------------  
SET @vchSP_name = 'spLocal_PositiveRelease'  
SET @vchRT_xlt = 'QA_Positive_Release_Report.xlt'  
-----------------------------------------------------------------------------------------------------------  
-- Search Version in AppVersions  
-----------------------------------------------------------------------------------------------------------  
SELECT @vchAppVersion = App_Version FROM dbo.AppVersions WITH(NOLOCK) WHERE App_Name LIKE @vchSP_name   
-----------------------------------------------------------------------------------------------------------  
-- Search Version in Report_Types  
-----------------------------------------------------------------------------------------------------------  
SELECT @vchRTVersion = Version FROM dbo.Report_Types WITH(NOLOCK) WHERE Template_Path LIKE '%' + @vchRT_xlt + '%'  
-------------------------------------------------------------------------------------------  
PRINT ' @strQualityUnit: ' + @strQualityUnit  
PRINT ' @PLIdList:       ' + @PLIdList  
PRINT ' @int_RptGroupBy: ' + STR(@int_RptGroupBy,1,0)  
...
SELECT @intTableFieldId = Table_Field_Id  
 ...
 WHERE Table_Field_Desc = @vchUDPDescIsTestComplete  --@vchUDPDescDefaultQProdGrps  
 ...
 BEGIN  
  ...
  -- Attr Issues  
     ...
     SELECT DISTINCT   at.PU_Id,  
        ...
        at.Var_Id  
  FROM @AttrTests at  
  JOIN @ProdUnitPrdPath pupp   ON at.PU_Id = pupp.PU_Id  
  JOIN dbo.Variables_Base v WITH(NOLOCK) ON v.Var_Id = at.Var_Id  
...
--  END  
  ...
  WHERE pupp.PL_Id = @PLId  
    ...
    AND (( a.Action1 IS NULL  
      AND a.Cause1 IS NULL  
      AND a.Ack <> 1  )  
...
-- SELECT '@AttrTests',* FROM @AttrTests  
-- SELECT '@QFactorTests',* FROM @QFactorTests  
-- SELECT '@TotalTests',* FROM @TotalTests  
-- SELECT '@OpenIssues',* FROM @OpenIssues  
-- SELECT '@RS2Summary',* FROM @RS2Summary ORDER BY PUId,PUGId  
-- SELECT '@RS3Summary',* FROM @RS3Summary  
-- SELECT '@RS4Summary',* FROM @RS4Summary  
-- SELECT '@RS5Summary',* FROM @RS5Summary  
-- SELECT '@RS6Summary',* FROM @RS6Summary  
-- SELECT '@RS7Summary',* FROM @RS7Summary  
-------------------------------------------------------------------------------------------------------------  
-------------------------------------------------------------------------------------------------------------  
...
GRANT EXECUTE ON [dbo].[spLocal_PositiveRelease] TO [OpDBManager] As [dbo]
GO
